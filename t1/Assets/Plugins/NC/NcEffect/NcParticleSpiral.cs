...
{ 
	...
	protected	const int	Max_numArms			= 20;
	protected	const int	Min_numPPA			= 1;
	protected	const int	Max_numPPA			= 800;
	protected	const float	Min_partSep			= -1.0f;
	protected	const float	Max_partSep			= 1.0f;
	protected	const float	Min_turnDist		= -1.0f;
	protected	const float	Max_turnDist		= 1.0f;
	protected	const float	Min_vertDist		= -1.0f;
	protected	const float	Max_vertDist		= 1.0f;
	protected	const float	Min_originOffset	= -10.0f;
	protected	const float	Max_originOffset	= 10.0f;
	protected	const float	Min_turnSpeed		= -200.0f;
	protected	const float	Max_turnSpeed		= 200.0f;
	protected	const float	Min_fade			= 0.0f;
	protected	const float	Max_fade			= 1.0f;
	protected	const float	Min_size			= -0.1f;
	protected	const float	Max_size			= 0.1f;
	...
	void SpawnEffect()
	{
		...
		GameObject effectObject = CreateGameObject(m_EffectPrefab, this.transform.position, this.transform.rotation);
	...
	void Update()
	{
		...
		this.transform.Rotate(this.transform.up * Time.deltaTime * (-m_fTurnSpeed), Space.World);
	}
	...
	void LateUpdate() 
	{
		...
		if (timeSinceLastSpawn >= m_fSpawnRate && spawnCount < m_nNumberOfSpawns)
		{
			SpawnEffect();
	...
	public SpiralSettings randomizeEffect(bool killCurrent)
	{
		...
		m_nNumberOfArms = Random.Range(Min_numArms, Max_numArms+1);  		
		m_nParticlesPerArm =   Random.Range(Min_numPPA, Max_numPPA+1); 
